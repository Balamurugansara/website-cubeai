...
// lightweight fallback for missing voice keywords
const VOICE_AGENT_KEYWORDS_STRING = '';
...
const ArticlePage = () => {
  ...
  useEffect(() => {
    ...
    const foundArticle = articles.find((a: any) => a.slug === id || a.id === id);
    if (foundArticle) {
      setArticle(foundArticle as Article);
      setRelatedArticles(
        articles.filter((a: any) => (foundArticle.relatedArticles || []).includes(a.id)) as Article[]
      );
      window.scrollTo(0, 0);
    } else {
      // don't redirect aggressively in client; let server handle 404s
      // but navigate back to blogs if not found
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/resources/blogs" className="text-purple-600 hover:text-purple-700">
            Return to Blogs
          </Link>
        </div>
      </div>
        ...
        <div className="container mx-auto px-4 lg:px-6">
          <div className="max-w-4xl mx-auto">
            <nav className="flex items-center space-x-2 text-sm text-gray-500 mb-8">
              ...
              <span>/</span>
              <Link href="/resources/blogs" className="hover:text-purple-600">
                Blogs
              </Link>
              <span>/</span>
            ...
            <div className="flex flex-wrap items-center gap-6 text-gray-500 mb-8">
              ...
              <div className="ml-auto flex items-center gap-3">
                <Link href="/resources/blogs" className="text-sm text-purple-600 hover:underline">
                  View all Blogs
                </Link>
                <Link href="/resources/case-studies" className="text-sm text-purple-600 hover:underline">
            ...
            <div className="flex items-center space-x-4 p-6 bg-white rounded-2xl shadow-lg border border-gray-100">
              <img
                src={(article as any).author?.avatar || '/default-avatar.png'}
            ...
            <div className="prose prose-lg prose-purple max-w-none">
              ...
              {article.flowchartImage && (
                <div className="my-16 bg-gray-50 p-6">
                  ...
                  />
                  <p className="text-center text-sm text-gray-500 mt-4">Figure: Workflow of AI Agent Collaboration</p>
                </div>
              )}
            ...
            <div className="max-w-6xl mx-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {relatedArticles.slice(0, 3).map((rel) => (
                  <Link
                    key={(rel as any).id}
                    href={`/resources/blogs/${(rel as any).slug}`}
                    className="group bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 overflow-hidden border border-gray-100"
                    aria-label={`Read article: ${(rel as any).title}`}
                  >
                    <div className="aspect-video bg-gray-200 overflow-hidden">
                      <img
                        src={(rel as any).image || '/default-image.png'}
                        alt={(rel as any).title}
                    ...
                    <div className="p-6">
                      ...
                      <p className="text-gray-600 mb-4 leading-relaxed line-clamp-3">{(rel as any).excerpt}</p>
